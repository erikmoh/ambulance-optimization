...
public class FourthExperiment implements Experiment {
  ...
  private static final PopulationProportionate populationProportionate =
  ...
  public void run() {
    ...
    logger.info(
        "Testing day ambulances between {} and {} with day to night ambulance ratio {}...",
        minAmbulances,
        maxAmbulances,
        DAY_TO_NIGHT_AMBULANCE_RATIO);
    logger.info("Using model 'PopulationProportionate'");

    IntStream.range(minAmbulances, maxAmbulances)
        .parallel()
        .forEach(
            numDayAmbulances -> {
              int numNightAmbulances =
                  (int) Math.round(numDayAmbulances * DAY_TO_NIGHT_AMBULANCE_RATIO);
              logger.info(
                  "Running simulation with {} day ambulances and {} night ambulances",
                  numDayAmbulances,
                  numNightAmbulances);
    ...
    logger.info(
        "Testing all day and night ambulance combinations between {} and {}...",
        minAmbulances,
        maxAmbulances);
    IntStream.range(minAmbulances, maxAmbulances)
        .parallel()
  ...
  public void saveResults() {
    averageResponseTimesResult1.saveColumn(
        "num_ambulances", IntStream.range(minAmbulances, maxAmbulances).boxed().toList());
    averageResponseTimesResult1.saveColumn(
        "PopulationProportionate",
        IntStream.range(minAmbulances, maxAmbulances)
            .mapToObj(averageResponseTimes1::get)
            .toList());
    averageResponseTimesResult1.saveColumn(
        "GA",
        IntStream.range(minAmbulances, maxAmbulances)
            .mapToObj(averageResponseTimes2::get)
            .toList());
    averageResponseTimesResult1.saveResults("fourth_experiment_average_response_times_ratio");
