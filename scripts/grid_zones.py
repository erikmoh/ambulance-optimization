...
def process_grids():
  ...
  empty_cells = pd.read_csv('data/empty_cells.csv', encoding='utf-8',
                            index_col=0)
  empty_cells = empty_cells[['X', 'Y']].rename(
...
def process_base_stations():
  base_stations = pd.read_csv('data/base_stations.csv', encoding='utf-8',
                              index_col=0)
